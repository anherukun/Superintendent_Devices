const char reburst_html[] PROGMEM = R"rawliteral(
    <!DOCTYPE html>
    <html>
    
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title>Formulario ESP32</title>
        <link href="/css/bootstrap.min.css" rel="stylesheet">
        <script src="/js/bootstrap.bundle.min.js"></script>
        <script src="/components/app-navbar.js"></script>
    </head>
        
<body data-bs-theme="dark">
    <app-navbar />

    <div class="container-sm">
        <h1 class="mt-2">Rafaga Repetitiva</h1>
        <p class="lead">Configura los parametros</p>

        <div class="d-grid gap-3 mb-3 ">

            <form class="d-grid gap-3 mb-3" action="/pgrm/load" method="GET">
                <div hidden>
                    <label class="form-label" for="mode">Programa</label>
                    <input class="form-control" type="number" id="mode" name="mode" value="4" required>
                </div>

                <div>
                    <label class="form-label" for="exp_t">Tiempo de Exposicion</label>
                    <select id="exp_t" class="form-select" name="exp_t" required>
                        <option selected disabled value="">Seleccione un tiempo</option>
                    </select>

                    <script>
                        const select = document.getElementById('exp_t');
                        for (let segundos = 1; segundos <= 5; segundos += 1) {
                            const option = document.createElement('option');
                            option.value = segundos * 1000; // Convertir segundos a milisegundos
                            option.textContent = `${segundos} segundos`;
                            select.appendChild(option);
                        }
                        for (let segundos = 10; segundos <= 60; segundos += 10) {
                            const option = document.createElement('option');
                            option.value = segundos * 1000; // Convertir segundos a milisegundos
                            option.textContent = `${segundos} segundos`;
                            select.appendChild(option);
                        }
                        for (let minutos = 1; minutos <= (5); minutos += 1) {
                            const option = document.createElement('option');
                            option.value = minutos * 60000; // Convertir minutos a milisegundos
                            option.textContent = `${minutos} minutos`;
                            select.appendChild(option);
                        }
                    </script>
                </div>

                <div>
                    <label class="form-label" for="burst_c">Numero total de disparos</label>
                    <input class="form-control" list="datalistOptions" type="number" min="0" value="0" id="burst_c" name="burst_c"
                        required>
                    <div class="alert alert-light" role="alert">
                        Nota: 0 equivale a disparar continuamente hasta que se detenga manualmente.
                    </div>
                </div>                

                <div>
                    <label class="form-label" for="burst_tb">Tiempo entre disparos</label>
                    <select id="burst_tb" class="form-select" name="burst_tb" required>
                        <option selected disabled value="">Seleccione un tiempo</option>
                    </select>

                    <script>
                        const select_burst_tb = document.getElementById('burst_tb');
                        for (let segundos = 1; segundos <= 5; segundos += 1) {
                            const option = document.createElement('option');
                            option.value = segundos * 1000; // Convertir segundos a milisegundos
                            option.textContent = `${segundos} segundos`;
                            select_burst_tb.appendChild(option);
                        }
                        for (let segundos = 10; segundos <= 50; segundos += 10) {
                            const option = document.createElement('option');
                            option.value = segundos * 1000; // Convertir segundos a milisegundos
                            option.textContent = `${segundos} segundos`;
                            select_burst_tb.appendChild(option);
                        }
                        for (let minutos = 5; minutos <= (30); minutos += 5) {
                            const option = document.createElement('option');
                            option.value = minutos * 60000; // Convertir minutos a milisegundos
                            option.textContent = `${minutos} minutos`;
                            select_burst_tb.appendChild(option);
                        }
                    </script>
                </div>

                <div>
                    <label class="form-label" for="reburst_c">Numero de repeticiones</label>
                    <input class="form-control" list="datalistOptions" type="number" min="0" id="reburst_c" name="reburst_c" value="0"
                        required>
                </div>

                <input class="btn btn-secondary w-100" type="submit" value="Enviar">
            </form>
        </div>
    </div>
    <script src="/js/bootstrap.min.js"></script>
</body>
        
    </html>
    )rawliteral";